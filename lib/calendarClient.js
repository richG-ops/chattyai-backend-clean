...
async function availability(params = {}) {
  if (providerIsCalcom() && calcom) {
    try {
      ...
      if (!fallbackAllowed) throw mapCalcomError(err);
      console.warn('calendar.availability: calcom failed → trying legacy', err?.response?.status || err?.code || err?.message);
    }
  }
  const baseURL = getBaseURL();
  if (!baseURL) {
...
async function book(startISO, endISO = null, payload = {}) {
  if (providerIsCalcom() && calcom) {
    try {
      ...
      if (!fallbackAllowed) throw mapCalcomError(err);
      console.warn('calendar.book: calcom failed → trying legacy', err?.response?.status || err?.code || err?.message);
    }
  }
  const baseURL = getBaseURL();
  if (!baseURL) {
