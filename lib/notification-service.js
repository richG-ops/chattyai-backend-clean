...
function initializeProviders() {
  ...
  try {
    if (process.env.SENDGRID_API_KEY) {
      ...
      console.log('✅ Notification Service: SendGrid initialized');
    } else {
      throw new Error('Missing SendGrid API key');
    }
  } catch (error) {
    console.warn('⚠️  SendGrid unavailable - using mock:', error.message);
...
class NotificationService {
  ...
  async sendSMS(to, template, data, options = {}) {
    ...
    if (typeof template === 'string') {
      ...
      const templateFn = templates.sms[template][data.recipient || 'customer'];
    ...
    // Rate-limited send
    return smsLimiter.schedule({ priority: priority === 'high' ? 1 : 5 }, async () => {
      let lastError;
      
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          console.log(`📱 Sending SMS (attempt ${attempt}/${retries}) to ${cleanPhone}`);
          ...
          const message = await twilio.messages.create({
            ...
            statusCallback: process.env.WEBHOOK_URL ? 
              `${process.env.WEBHOOK_URL}/api/notifications/status` : undefined
          });
          ...
          await this.logNotification({
            ...
            category,
            metadata: { attempt, messageId: message.sid }
          });
