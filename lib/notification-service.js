...
function initializeProviders() {
  ...
  // Telnyx lazy import (only if configured)
  try {
    if (process.env.TELNYX_API_KEY && process.env.TELNYX_MESSAGING_PROFILE_ID) {
      telnyxProvider = require('./providers/telnyx');
      console.log('✅ Notification Service: Telnyx adapter ready');
    }
  } catch (error) {
    console.warn('⚠️  Telnyx adapter unavailable:', error.message);
    telnyxProvider = null;
  }
...
class NotificationService {
  ...
  async sendSMS(to, template, data, options = {}) {
    ...
    if (!e164 || !/^\+\d{8,15}$/.test(e164)) {
      throw new Error('INVALID_TO_E164');
    }
    // central suppression check
    try {
      const { getDb } = require('../db-config');
      const db = getDb();
      const sup = await db.raw('select suppressed from sms_suppressions where to_e164 = ? limit 1', [e164]);
      const row = sup?.rows?.[0] || sup?.[0];
      if (row && (row.suppressed === true || row.suppressed === 't')) {
    ...
    // Log provider order for observability
    try { console.log('[sms.providers.order]', { to: e164, order }); } catch {}
    ...
    const attemptSend = async (provider) => {
      ...
      if (provider === 'telnyx' && telnyxProvider) {
        return await telnyxProvider.sendSMS({ toE164: e164, body });
      }
      ...
      try {
        return await promiseRetry(async (retry, n) => {
          try {
            ...
            try {
              const acceptedId = res?.sid || res?.providerMessageId || res?.id || res?.requestId;
              console.log('[sms.accepted.by.provider]', { to: e164, provider, id: acceptedId });
            } catch {}
            return res;
          } catch (err) {
