...
// Environment
const RAW_API_BASE = process.env.CAL_API_BASE || 'https://api.cal.com';
const API_BASE = RAW_API_BASE.replace(/\/v\d+\/?$/, '');
...
function normalizeSlots(data) {
  ...
  if (!arr.length && data && typeof data === 'object' && !Array.isArray(data)) {
    // Case A: top-level date-keyed map { "2025-09-01": [slots], ... }
    let values = Object.values(data).filter(Array.isArray);
    // Case B: nested under data: { data: { "2025-09-01": [slots], ... } }
    if (!values.length && data.data && typeof data.data === 'object') {
      values = Object.values(data.data).filter(Array.isArray);
    }
    if (values.length) {
      arr = values.flat();
    }
