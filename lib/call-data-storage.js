...
class CallDataStorage {
  ...
  async storeCallData(callData, options = {}) {
    ...
    try {
      ...
      const businessId = this.validateBusinessId(callData.business_id);
      await client.query('SET app.tenant_id = $1', [businessId]);
      
      // Validate and sanitize all input data
      const validatedData = {
        business_id: businessId,
        call_id: callData.call_id || null,
        vapi_assistant_id: callData.vapi_assistant_id || null,
        caller_phone: this.validatePhone(callData.caller_phone),
        caller_email: this.validateEmail(callData.caller_email),
        customer_name: callData.customer_name ? callData.customer_name.trim() : null,
        appointment_date: this.validateAppointmentDate(callData.appointment_date),
        appointment_time: callData.appointment_time || null,
        service_type: callData.service_type || null,
        status: callData.status || 'pending',
        raw_vapi_payload: callData.raw_vapi_payload ? JSON.stringify(callData.raw_vapi_payload) : null,
      ...
      const storedData = { ...validatedData, ...result.rows[0] };

      // Commit transaction
      await client.query('COMMIT');

      // Log successful storage
      console.log(`✅ Call data stored successfully: ${storedData.id}`, {
        business_id: businessId,
      ...
      this.fireAsyncEvents(storedData).catch(err => {
